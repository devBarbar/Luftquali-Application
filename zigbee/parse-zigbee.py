...
def fake_data(): 
    ...
    for i in range(0,3):
        if i == 0: values = [random.randint(0,100) for _ in range(20)]
        elif i ==1: values = [random.randint(0,40) for _ in range(20)]
        elif i ==2: values = values = [random.randint(400,1200) for _ in range(20)]
        result.append(json.dumps({"SENSOR":"%s","TYPE":"%s","VALUES":"%s"}) % (sensorID[i],types[i],values))
    time.sleep(10)
